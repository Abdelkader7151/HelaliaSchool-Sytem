...
$typeLabel = function ($t) {
    $t = (int) $t;
    if ($t === 1) return 'ØºÙŠØ§Ø¨';
    if ($t === 6) return 'Ø¥Ø¹Ù„Ø§Ù†';
    if ($t === 65) return 'Ø±Ø³Ø§Ù„Ø©';
    return 'ØªÙ†Ø¨ÙŠÙ‡';
};
...
<title>Ø§Ù„ØªÙ†Ø¨ÙŠÙ‡Ø§Øª Â· Ù‡Ù„Ø§Ù„ÙŠØ§</title>
...
?>
      ...
      <h1 class="hero__title">Ø§Ù„ØªÙ†Ø¨ÙŠÙ‡Ø§Øª</h1>
      <div class="bells"></div>
    </div>
  </header>
...
if (count($tabRows) > 1) { ?>
    <div class="tabs tabs--scroll">
      <a class="tab<?php echo $kid_id < 1 ? ' is-active' : ''; ?>" href="parent-alerts.php">Ø§Ù„ÙƒÙ„</a>
...
<?php } ?>
    ...
    <p class="auth__fine" style="margin:0 0 10px"><?php if ($unreadNew > 0) { echo $unreadNew . ' Ø¬Ø¯ÙŠØ¯Ø©'; } else { echo 'Ù„Ø§ ØªÙˆØ¬Ø¯ ØªÙ†Ø¨ÙŠÙ‡Ø§Øª Ø¬Ø¯ÙŠØ¯Ø©'; } ?>.</p>
<?php if ($unreadNew > 0) {
    $markUrl = 'parent-alerts.php?mark_all=1' . ($kid_id > 0 ? '&kid=' . $kid_id : '');
?>
    <p style="margin:0 0 12px"><a class="btn btn--quiet" href="<?php echo htmlspecialchars($markUrl, ENT_QUOTES, 'UTF-8'); ?>">ØªØ¹ÙŠÙŠÙ† Ø§Ù„ÙƒÙ„ ÙƒÙ…Ù‚Ø±ÙˆØ¡</a></p>
<?php } ?>

    <div class="rows" id="alerts-list">
<?php if (!$rows) { ?>
      <p class="auth__fine">Ù„Ø§ ØªÙˆØ¬Ø¯ ØªÙ†Ø¨ÙŠÙ‡Ø§Øª Ø¬Ø¯ÙŠØ¯Ø©.</p>
<?php } else {
    foreach ($rows as $n) {
      ...
      <a class="<?php echo $rowClass; ?>" href="<?php echo htmlspecialchars($href, ENT_QUOTES, 'UTF-8'); ?>">
        ...
        <div class="row__body">
          ...
          <p class="row__meta"><?php
            if ($kid_id < 1) {
                echo htmlspecialchars(kid_name($n['kid_id']), ENT_QUOTES, 'UTF-8') . ' Â· ';
            }
            echo $typeLabel($n['type']);
          ?></p>
          <p class="row__time"><?php echo date('d M, Y', (int) $n['date']); ?></p>
        </div>
        <span class="row__go">â€º</span>
      </a>
<?php }
} ?>
    </div>
...
?>
    <a class="btn btn--quiet alerts-more" style="display:block;width:100%;text-align:center;margin:14px 0" href="<?php echo htmlspecialchars($moreUrl, ENT_QUOTES, 'UTF-8'); ?>">ØªØ­Ù…ÙŠÙ„ Ø§Ù„Ù…Ø²ÙŠØ¯</a>
<?php } ?>
  </main>
...
<?php if ($kid_id > 0) { ?>
  <nav class="nav" aria-label="Home" style="height:90px">
    <a class="nav__item" href="parent-view.php">
      ...
      <span>Ø§Ù„Ø·Ù„Ø§Ø¨</span><span class="nav__dot"></span>
    </a>
    <a class="nav__item" href="parent-calendar.php?kid=<?php echo $kid_id; ?>">
      ...
      <span>Ø§Ù„ØªÙ‚ÙˆÙŠÙ…</span><span class="nav__dot"></span>
    </a>
    ...
    <a class="nav__item" href="parent-timeline.php">
      ...
      <span>Ø¢Ø®Ø± Ø§Ù„Ø£Ø®Ø¨Ø§Ø±</span><span class="nav__dot"></span>
    </a>
    <a class="nav__item" href="parent-settings.php">
      ...
      <span>Ø§Ù„Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª</span><span class="nav__dot"></span>
    </a>
  </nav>
<?php } else { ?>
