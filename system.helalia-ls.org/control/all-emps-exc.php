...
if(isset($_GET['id'])){
  $query_get_kids_vacations = "SELECT * FROM `emps_excuse` where `status` = '{$_GET['id']}' order by `date` asc  "; 
}else{
  $query_get_kids_vacations = "SELECT * FROM `emps_excuse` order by `date` asc  "; 
} 
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
			<div class="row">
            <div class="col-md-12">
                <h4>عرض اذون <?php if(isset($_GET['id'])){echo "الجديدة";}?> للموظفين   </h4>  
            </div>
          </div>
 ...
 <script>
	  $(document).ready(function(){ 
		...
		$("#court-datatables").DataTable({ 
            ...
            "aoColumnDefs": [
               { "bSortable": false,
			     "aTargets": [ <?php if(!isset($_GET['id'])){echo 7;}else{echo 6;}?> ]  
			   }
			 ]
        }); 
