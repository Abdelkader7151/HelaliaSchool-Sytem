...
<div class="app">  
  ...
  <header class="hero hero--tall">
    <div class="hero__row">
        <a class="back" href="parent-kid.php?id=<?php echo $kid_id;?>" aria-label="رجوع">
         ...
         <h1 class="hero__title">الواجب المنزلي</h1>
         <div class="bells">
  ...
  <main class="page">
      ...
      <div class="tabs">
        <a class="tab is-active" href="parent-homework.php?id=<?php echo $row_get_kid_data['id'];?>">الواجب المنزلي</a>
        <a class="tab" href="parent-revision.php?id=<?php echo $row_get_kid_data['id'];?>">مراجعة</a> 
      </div>
 ...
 <?php 
  ...
  if($totalRows_get_subjects>0){ 
       ...
       do{ 
        $query_get_data = "SELECT COUNT(*) as `homework` FROM `homework` WHERE `study_year` = '{$row_get_kid_data['study_year']}' AND (`class` = '{$row_get_kid_data['class']}' ||  `class` = 0 ) AND `confirm` = 1 AND `subject` = '{$row_get_subjects['id']}' AND `date` ='{$today}' order BY `id` desc  ";
        ...
        <a class="row <?php echo getRandomColor(); ?>" href="parent-homework-view.php?id=<?php echo $row_get_subjects['id'];?>&kid=<?php echo $row_get_kid_data['id'];?>">
           <div class="row__body">
             <p class="row__title"><?php echo $row_get_subjects['name']; ?></p>
           </div>
        <?php if($row_get_data['homework']>0){ ?>
          <span class="row__count row__count--gold"><?php echo $row_get_data['homework']; ?></span>
        <?php } ?>
        </a>
