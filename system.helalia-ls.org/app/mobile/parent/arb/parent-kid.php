                          ...
                          if ($old_picture && $old_picture !== 'no-picture.png') {
                              ...
                              if (file_exists($old_path)) {
                                  if (is_writable($old_path)) {
                                      ...
                                      if (!$deleted) {
                                          error_log('parent-kid.php (ar): failed to unlink old picture at ' . $old_path);
                                      }
                                  } else {
                                      error_log('parent-kid.php (ar): old picture not writable, cannot delete: ' . $old_path);
                                  }
                              } else {
                                  error_log('parent-kid.php (ar): old picture DB value did not match a file on disk: ' . $old_path);
                              }
                          }
...
<div class="app">
  <header class="hero hero--tall">
    ...
    <section class="featured featured--profile">
      <button type="button" class="av av--lg av--clickable" id="pic-modal-trigger" aria-label="تغيير الصورة" style="padding:0;border:none;background:none;">
      ...
      <div class="featured__body">
        ...
        <p class="featured__meta">ابتدائي <?php echo year_of_study($row_get_kid_info['study_year']); ?> · ⁦<?php echo class_name($row_get_kid_info['class']);?>⁩</p>
        <div class="featured__tags"><span class="tag">رقم الطالب: <?php echo $row_get_kid_info['ed_id']; ?></span></div>
      </div>
      </section>
  ...
  <main class="page">
    ...
    <div class="quickrow quickrow--3">
       <?php 
         ...
         <span class="quick__ico quick__ico--art"><img src="../assets/img/quick/ask.webp" alt="" aria-hidden="true"></span>اسأل المدرسة</a>
        
         <a class="quick t-blue" href="parent-calendar.php?kid=<?php echo $row_get_kid_info['id'];?>">
         <span class="quick__ico quick__ico--art"><img src="../assets/img/quick/calendar.avif" alt="" aria-hidden="true"></span>التقويم</a>
       
         <a class="quick t-green" href="parent-summary.php?kid=<?php echo $row_get_kid_info['id'];?>">
         <span class="quick__ico quick__ico--art"><img src="../assets/img/quick/attendance.webp" alt="" aria-hidden="true"></span>الغياب</a>
    </div>

    <div class="sec">
      <h2 class="sec__title">كل ما يخص الطالب</h2>
    </div>
        ...
        ?>
          <a class="tile t-coral" href="parent-homework.php?id=<?php echo $row_get_kid_info['id'];?>">
           <?php if($row_get_homework_count['homework']>0){ ?><span class="tile__new pulse"><?php echo $row_get_homework_count['homework']; ?></span><?php } ?>
            ...
            <span class="tile__label">الواجبات</span>
          </a>
      ...
      $totalRows_get_memo_count = mysqli_num_rows($get_memo_count); ?>
          <a class="tile t-blue" href="parent-memo.php?id=<?php echo $row_get_kid_info['id'];?>">
            ...
            <span class="tile__label">المذكرات</span>
          </a>
         ...
         <?php
          ...
          <a class="tile t-gold" href="parent-revision.php?id=<?php echo $row_get_kid_info['id'];?>">
            ...
            <span class="tile__label">المراجعة</span>
          </a>
          ...
          <a class="tile t-navy" href="parent-plan.php?kid=<?php echo $row_get_kid_info['id'];?>">
            ...
            <span class="tile__label">الخطة الأسبوعية</span>
          </a>
           ...
           <a class="tile t-coral" href="#">
            ...
            <span class="tile__label">المعرض</span>
          </a>
           ...
           <a class="tile t-green" href="#">
            ...
            <span class="tile__label">النتائج</span>
          </a>
  ...
  <div class="pic-modal" id="pic-modal">
    ...
    <div class="pic-modal__box" role="dialog" aria-modal="true" aria-labelledby="pic-modal-title">
      <button class="pic-modal__close" id="pic-modal-close" aria-label="إغلاق">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M18 6 6 18"/><path d="M6 6l12 12"/>
        </svg>
      </button>
      <h3 class="pic-modal__title" id="pic-modal-title">تغيير الصورة</h3>

      <p class="pic-modal__error<?php echo $upload_error ? ' show' : ''; ?>" id="pic-modal-error">
        <?php
        switch ($upload_error) {
            case 'too_large':    echo 'الصورة كبيرة جدًا. الحد الأقصى 5 ميجابايت.'; break;
            case 'bad_type':     echo 'من فضلك اختر صورة بصيغة JPG أو PNG أو WEBP.'; break;
            case 'upload_failed':
            case 'save_failed':  echo 'فشل رفع الصورة، برجاء المحاولة مرة أخرى.'; break;
        }
        ?>
      </p>

      <form action="parent-kid.php?id=<?php echo $kid_id;?>" method="post" enctype="multipart/form-data" id="pic-upload-form">
        <img class="pic-modal__preview" id="pic-preview-img" src="../../../../kids/<?php if($row_get_kid_info['picture']!=NULL && file_exists('../../../../kids/'.$row_get_kid_info['picture'])==1){echo $row_get_kid_info['picture'];}else{ echo "no-picture.png";} ;?>" alt="معاينة">
        ...
        <div class="pic-modal__actions">
          <button type="button" class="btn btn--quiet" id="pic-modal-cancel">إلغاء</button>
          <button type="submit" class="btn btn--primary" id="pic-modal-save">حفظ</button>
        </div>
      </form>
    </div>
  </div>
...
picForm.addEventListener('submit', function () {
  ...
  picSaveBtn.textContent = 'جارٍ الحفظ...';
});
