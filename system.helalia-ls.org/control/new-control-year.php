<?php require_once('includes/access.php'); 
 ...
 if(isset($_POST['submit'])){
        ...
        $query_get_kids = "SELECT * FROM `kids` WHERE `study_year`= '{$_POST['study_year']}' $class $gender  ";
        ...
        $totalRows_get_subject = mysqli_num_rows($get_subject); 
            ...
            do{  
                mysqli_select_db($database, $database_database); 
                $query_get_control = "SELECT * FROM `control_year` WHERE `kid_id` = '{$row_get_kids['id']}' AND `study_year`= '{$row_get_kids['study_year']}' AND `class`= '{$row_get_kids['class']}' AND `subject_id` = '{$_POST['subject']}'  ";
                ...
                $totalRows_get_control = mysqli_num_rows($get_control);
                        if($totalRows_get_control<1){ 
                            $insertSQL = sprintf("INSERT INTO `control_year` ( `kid_id`, `name`, `arb_name`, `gender`, `study_year`, `class`, `subject_id`, `subject_name`, `date`, `phase1_total`, `phase2_total` ) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )", 
                                            ...
                                            GetSQLValueString($database,$row_get_subject['phase1_total'], "double"),   
                                            GetSQLValueString($database,$row_get_subject['phase2_total'], "double"));
        ...
        header("location: control-year-step2.php?year=".$_POST['study_year']."&class=".$_POST['class']."&gender=".$_POST['gender']."&subject=".$_POST['subject']);
        exit();
 } 
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
			<div class="row">
            <div class="col-md-12">
                <h4>     رصد اعمال السنة </h4>
            </div>
          </div>
			<div class="row">
				<div class="col-md-8">
				  <div class="demo-form-wrapper">
                    <form action="new-control-year.php" method="post" name="form1" id="demo-inputmask"  enctype="multipart/form-data" class="form form-horizontal">
