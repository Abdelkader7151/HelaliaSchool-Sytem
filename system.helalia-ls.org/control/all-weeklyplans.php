<?php require_once('includes/access.php'); 
 ...
 if($row_get_login['access20sub2']==1){
  ...
  if(isset($_GET['del'])){   
    ...
    $query_get_class_info = "SELECT * FROM `weeklyplan`  WHERE `id` = '{$_GET['del']}'  "; 
    ...
    if($row_get_class_info['banner']!=NULL){ unlink('../homework/'.$row_get_class_info['banner']); }
       ...
       $deleteSQL = sprintf("DELETE FROM `weeklyplan` WHERE `id`=%s ",
        ...
        header("location: all-weeklyplans.php"); 
        exit(); 
} 
...
mysqli_select_db($database, $database_database);  
$query_get_class_info = "SELECT * FROM `weeklyplan` ORDER BY `id` DESC "; 
$get_class_info = mysqli_query($database,$query_get_class_info) or die(mysqli_error($database));
$row_get_class_info = mysqli_fetch_assoc($get_class_info);
$totalRows_get_class_info = mysqli_num_rows($get_class_info);
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
            ...
            <div class="col-md-12">
                <h4 style="padding-right: 20px;">عرض جميع الخطط الاسبوعية   
                  <br>
                  <a href="all-homeworks.php?all" style="float:left; width:100px" class="btn btn-primary btn-block " > عرض الجميع </a>
                </h4>
            </div>
          </div>
			<div class="row">
                ...
                <table id="court-datatables" class="table table-striped table-nowrap dataTable" cellspacing="0" width="100%" style="font-size: 16px">
                    ...
                    <tbody>
                      ...
                      <tr> 
                        ...
                        <td class="text-center"><?php echo $row_get_class_info['name_arb']." - ".$row_get_class_info['name_eng']." - ";?>
                        ...
                        <td class="text-center">
                          <?php if($row_get_class_info['emp_id']>0){echo users_name($row_get_class_info['emp_id']);} 
                                if($row_get_class_info['app_id']>0){echo parent_app($row_get_class_info['app_id']);} ?></td> 
                          ...
                          <div class="dropdown">
                            <button class="btn btn-primary btn-xs dropdown-toggle" data-toggle="dropdown" type="button" aria-expanded="false">
                              ...
                              <ul class="dropdown-menu dropdown-menu-right"> 
                                 <li><a href="all-weeklyplans.php?del=<?php echo $row_get_class_info['id'];?>" onClick="return confirm('تاكيد؟');">  <i class="fa fa-trash-o" aria-hidden="true" style="color: red"></i>  حذف </a></li>
                             </ul>
                          </div>
                        </td>
                      </tr>
