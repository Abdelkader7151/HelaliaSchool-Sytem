...
portal_head(t('homework'));
portal_top($user, $kid);
echo '<main class="page"><div class="wrap">';
echo '<h1 class="page-title">' . h(t('homework')) . '</h1>';
$shown = 0;
foreach ($subjects as $sub) {
    if ($first) {
        $rows = db_all(
            'SELECT * FROM `homework` WHERE `study_year` = ? AND (`class` = ? OR `class` = 0) AND `confirm` = 1 AND `subject` = ? AND `start` <= ? ORDER BY `id` DESC',
            'iiii',
