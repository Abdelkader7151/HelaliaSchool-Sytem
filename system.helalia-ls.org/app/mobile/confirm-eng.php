...
if($totalRows_get_reset>0){
    header("location:reset-eng.php?code=".$code);
    exit();
}else{$wrong = 1;}
...
<div class="app app--auth"><div class="auth">
    ...
    <div class="auth__brand">
      ...
      <h1 class="auth__name">Reset Code Sent by Email</h1>
      <p class="auth__tag">Enter the code to continue.</p>
    </div>
    
    
    <form class="auth__form" method="post" autocomplete="on" action="confirm-eng.php?id=<?php echo $_SESSION['phone_id'];?>">
      <label class="field"><span class="field__label">Reset Code</span>
      <input class="input" type="text" inputmode="numeric" placeholder="••••" required name="code" minlength="10" style="text-align:center"></label>
      <button class="btn btn--gold"  name="submit"  type="submit">Confirm Code</button>
      <br>
...
<?php if ($wrong == 1) { ?> 
    ...
    <div class="hl-modal" id="wrong-password" role="alertdialog" aria-live="assertive" aria-label="Login error">
      <div class="hl-modal__box">
        ...
        <p class="hl-modal__text">  Wrong Code    </p>
      </div>
    </div>
