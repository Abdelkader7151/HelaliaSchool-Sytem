...
$typeLabel = function ($t) {
    ...
    if ($t === 6) return 'Announcement';
    if ($t === 65) return 'Message';
    return 'Alert';
};
...
if (count($tabRows) > 1) { ?>
    <div class="tabs tabs--scroll">
      <a class="tab<?php echo $kid_id < 1 ? ' is-active' : ''; ?>" href="parent-alerts.php">All</a>
<?php foreach ($tabRows as $tr) {
    $kd = mysqli_query($database, "SELECT `id`,`fn_name` FROM `kids` WHERE `id` = '{$tr['kid_id']}' LIMIT 1");
    $kidRow = $kd ? mysqli_fetch_assoc($kd) : null;
    if (!$kidRow) continue;
?>
      <a class="tab<?php echo ($kid_id === (int) $kidRow['id']) ? ' is-active' : ''; ?>" href="parent-alerts.php?kid=<?php echo (int) $kidRow['id']; ?>"><?php echo htmlspecialchars($kidRow['fn_name'], ENT_QUOTES, 'UTF-8'); ?></a>
<?php } ?>
    </div>
<?php } ?>

    <p class="auth__fine" style="margin:0 0 10px">Showing the last <?php echo (int) helalia_alert_new_days(); ?> days<?php if ($unreadNew > 0) { echo ' · ' . $unreadNew . ' new'; } ?>.</p>
<?php if ($unreadNew > 0) {
    $markUrl = 'parent-alerts.php?mark_all=1' . ($kid_id > 0 ? '&kid=' . $kid_id : '');
?>
    <p style="margin:0 0 12px"><a class="btn btn--quiet" href="<?php echo htmlspecialchars($markUrl, ENT_QUOTES, 'UTF-8'); ?>">Mark all as read</a></p>
<?php } ?>

    <div class="rows" id="alerts-list">
<?php if (!$rows) { ?>
      <p class="auth__fine">No new alerts in this period.</p>
<?php } else {
    foreach ($rows as $n) {
...
?>
    <a class="btn btn--quiet alerts-more" style="display:block;width:100%;text-align:center;margin:14px 0" href="<?php echo htmlspecialchars($moreUrl, ENT_QUOTES, 'UTF-8'); ?>">Load more</a>
<?php } ?>
  </main>
...
<?php if ($kid_id > 0) { ?>
  <nav class="nav" aria-label="Home" style="height:90px">
    <a class="nav__item" href="parent-view.php">
      ...
      <span>Students</span><span class="nav__dot"></span>
    </a>
    <a class="nav__item" href="parent-calendar.php?kid=<?php echo $kid_id; ?>">
      ...
      <span>Calendar</span><span class="nav__dot"></span>
    </a>
    <a class="nav__fab" href="parent-kid.php?id=<?php echo (int) $row_get_kid_data['id']; ?>" aria-label="<?php echo htmlspecialchars($row_get_kid_data['fn_name'], ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <a class="nav__item" href="parent-timeline.php">
      ...
      <span>Latest News</span><span class="nav__dot"></span>
    </a>
    <a class="nav__item" href="parent-settings.php">
      ...
      <span>Settings</span><span class="nav__dot"></span>
    </a>
  </nav>
<?php } else { ?>
