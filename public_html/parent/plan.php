...
if ($first) {
    $rows = db_all(
        'SELECT * FROM `weeklyplan` WHERE `study_year` = ? AND `confirm` = 1 AND (`class` = ? OR `class` = 0) AND `date` <= ? ORDER BY `id` DESC',
        'iii',
...
portal_head(t('weekly_plan'));
portal_top($user, $kid);
echo '<main class="page"><div class="wrap">';
echo '<h1 class="page-title">' . h(t('weekly_plan')) . '</h1>';
if (!$rows) {
    empty_state(t('empty'));
} else {
    echo '<div class="list">';
    foreach ($rows as $row) {
        ...
        $when = (int) ($row['date'] ?? $row['start'] ?? 0);
        if ($when) {
            echo '<time>' . h(date('d/m/Y', $when)) . '</time>';
        }
