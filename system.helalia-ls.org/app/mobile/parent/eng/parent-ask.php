...
mysqli_select_db($database , $database_database,);
$query_get_question = "SELECT * FROM `ask_teacher` where `kid_id` = '{$kid_id}' AND (`del` IS NULL OR `del` = 0) order by `id` desc   ";
...
<div class="app">
  ...
  <main class="page">
    <div class="card stack">   
        <button class="btn btn--primary" type="button" id="new-message-btn">Send New Message</button>
      </div>
 ...
 $loadmore = 0;
  ...
  $loadmore = $totalRows_get_question; ?>
      <div class="sec">
        <h2 class="sec__title">My questions</h2>
      </div>
...
<?php $i = 1;
 ...
 <?php $i++; }while($row_get_question = mysqli_fetch_assoc($get_question)); ?>
  ...
  <?php if($loadmore>5){ ?>
      <button class="btn btn--quiet qa__more" id="qa-more" type="button">Load older messages</button>
      <?php } ?>
 <?php }?>
 ...
 <nav class="nav" aria-label="Home"  style="height: 90px">
  ...
  <div class="qa-modal" id="qa-modal">
    ...
    <div class="qa-modal__box" role="dialog" aria-modal="true" aria-labelledby="qa-modal-title">
      ...
      <p class="qa-modal__waiting" id="qa-modal-waiting">Waiting for a reply from the school.</p>

      <div class="qa-modal__delete-wrap">
        <button class="btn btn--danger" id="qa-modal-delete" type="button">Delete question</button>
  ...
  <div class="new-msg-modal" id="new-message-modal">
    ...
    <div class="new-msg-modal__box" role="dialog" aria-modal="true" aria-labelledby="new-message-title">
      ...
      <h3 class="new-msg-modal__title" id="new-message-title">Ask The School</h3>
      <div id="new-message-body">
        <div class="  clients-row">
            ...
            $query_get_subjects = "SELECT * FROM `subjects` WHERE `study_year` = '{$ask_study_year}' ORDER BY `name_eng` ASC, `name` ASC, `id` ASC";
        ...
        <ul class="collapsible">
          <li>
            <div class="collapsible-header is-open"><i class="fa fa-book" aria-hidden="true"></i> Coordinator</div>
            <div class="collapsible-body is-open">
              <div class="clients-row">
                <?php foreach ($ask_coord_subjects as $row_get_subjects) {
                    ...
                    if ($subjLabel === '') {
                        $subjLabel = 'Subject #' . (int) $row_get_subjects['id'];
                    }
                ?>
                        <div class="col s6">
                            <div class="client-box">
...
function openQaModal(item) {
  ...
  document.getElementById('qa-modal-time').textContent = 'Sent ' + item.dataset.sent;

  if (item.dataset.status === '1') {
    document.getElementById('qa-modal-from').textContent = 'Reply By \\ ' + item.dataset.teacher;
    document.getElementById('qa-modal-replytext').textContent = item.dataset.reply;
    document.getElementById('qa-modal-replydate').textContent = 'Replied ' + item.dataset.replydate;
  ...
  fetch(qaAjaxUrl, {
    ...
    .then(function (data) {
      if (data && data.success) {
        ...
        resetDeleteButton();
        deleteErrorEl.textContent = 'Could not delete this question. Please try again.';
