<?php require_once('includes/access.php'); 
 ...
 if($row_get_login['access19sub2']==1){
  ...
  if(isset($_GET['del'])){   
        ...
        $query_get_class_info = "SELECT * FROM `homework`  WHERE `id` = '{$_GET['del']}'  "; 
        ...
        if($row_get_class_info['banner']!=NULL){ unlink('../homework/'.$row_get_class_info['banner']); }
           ...
           $deleteSQL = sprintf("DELETE FROM `homework` WHERE `id`=%s ",
            ...
            header("location: search-homeworks.php?date=".$_GET['date']."&year=".$_GET['year']); 
            exit(); 
    } 
...
if($_GET['year']==100){$year = "";}else{ $year = " AND `study_year` = '{$_GET['year']}' ";}

mysqli_select_db($database, $database_database);  
$query_get_class_info = "SELECT * FROM `homework`  WHERE `start` = '{$start}' $year "; 
$get_class_info = mysqli_query($database,$query_get_class_info) or die(mysqli_error($database));
$row_get_class_info = mysqli_fetch_assoc($get_class_info);
$totalRows_get_class_info = mysqli_num_rows($get_class_info);
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
			<div class="row">
            <div class="col-md-12">
                <h4>عرض جميع الواجبات المدرسية     </h4>
            </div>
          </div>
			<div class="row">
            ...
            <div class="col-xs-12" style="padding: 20px;">  
                   <form method="get" name="form1" id="demo-inputmask"  enctype="multipart/form-data" class="form form-horizontal">
                     ...
                     <div class="form-group">
                        <label class="col-sm-1 control-label" for="date">  اليوم  </label>
                        <div class="col-sm-3">
                      ...
                      <div class="form-group">
						...
						<div class="col-sm-3">
                            <select class="form-control"   name="year" id="year" >
                                <option value="100" >جميع المراحل</option> 
                    ...
                    <tbody>
                      ...
                      <tr> 
                        ...
                        <td class="text-left"><?php echo $row_get_class_info['name_arb']." - ".$row_get_class_info['name_eng']." - ";?>
                          <?php if($row_get_class_info['banner']!=NULL){?>  <a href="../homework/<?php echo $row_get_class_info['banner'];?>">تحميل <i class="fa fa-download" aria-hidden="true"></i> </a> <?php }?></td> 
                          ...
                          <div class="dropdown">
                            <button class="btn btn-primary btn-xs dropdown-toggle" data-toggle="dropdown" type="button" aria-expanded="false">
                              ...
                              <ul class="dropdown-menu dropdown-menu-right"> 
                                 <li><a href="search-homeworks.php?del=<?php echo $row_get_class_info['id']; if(isset($_GET['date'])){echo "&date=".$_GET['date'];}if(isset($_GET['year'])){echo "&year=".$_GET['year'];}?>" onClick="return confirm('تاكيد؟');">  <i class="fa fa-trash-o" aria-hidden="true" style="color: red"></i>  حذف </a></li>
                             </ul>
                          </div>
                        </td>
                      </tr>
