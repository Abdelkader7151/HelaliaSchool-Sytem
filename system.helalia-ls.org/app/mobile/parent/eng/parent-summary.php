...
<div class="app">
  ...
  <div class="stats stats--4up">
    ...
    <div class="stat t-coral">
      <p class="stat__label" style="text-transform:uppercase">Total absence</p>
      <p class="stat__value"><?php echo $totalRows_get_kids_absence;?> Days</p>
    </div>

    <div class="stat t-green">
      <p class="stat__label" style="text-transform:uppercase">Accepted vacations</p>
      <p class="stat__value"><?php echo $row_get_kids_vacations_accepted['accepted'];?></p>
    </div>

    <div class="stat t-coral">
      <p class="stat__label" style="text-transform:uppercase">Not accepted</p>
    ...
    <div class=" " align="center" style="margin-top: 10px">  
      ...
      <i class="fa fa-plus fa-lg stat__label pulse " aria-hidden="true"   ></i> Submit vacation</a>
    </div>
 ...
 <div class="sec">
    <h2 class="sec__title">Absence</h2>
  </div>
 ...
 <?php if ($totalRows_get_kids_absence > 0) {
      ...
      <div class="row__body">
        ...
        <p class="row__meta">
         <?php if ($totalRows_get_kids_vacations > 0 && $row_get_kids_vacations['status']>0) { ?> 
          <?php if($row_get_kids_vacations['status']==1){echo "Reason Received"; } ?>
          <?php if($row_get_kids_vacations['status']==2){echo "Reason Rejected"; } ?>
          <?php }else{ echo "Submit reason for being absent"; } ?>
        </p> 
      </div>
        <?php if ($totalRows_get_kids_vacations > 0 && $row_get_kids_vacations['status']>0) { ?> 
          <?php if($row_get_kids_vacations['status']==1){?> <span class="chip chip--ok">Accepted</span> <?php } ?>
          <?php if($row_get_kids_vacations['status']==2){?> <span class="chip chip--no">Not accepted</span> <?php } ?>
          <?php }else{?>
          <span class="chip chip--wait">Pending Action</span>
       <?php } ?> 
    </a>
  ...
  <button class="load-more-btn" type="button" data-target="absence-rows">Load more</button>



  
 <div class="sec">
    <h2 class="sec__title">Vacation Submitted</h2>
  </div>
 ...
 <?php  
      ...
      <div class="row__body">
        ...
        <p class="row__meta"> 
         ...
         <?php if ( $row_get_kids_vacations['status']>0) { ?> 
          <?php if($row_get_kids_vacations['status']==1){?> <span class="chip chip--ok" style="text-align:center"><?php echo $days;?> Day<?php if($days>1){echo "s";}?><br>Accepted</span> <?php } ?>
          <?php if($row_get_kids_vacations['status']==2){?> <span class="chip chip--no" style="text-align:center"><?php echo $days;?> Day<?php if($days>1){echo "s";}?><br>Not accepted</span> <?php } ?>
          <?php }else{?>
          <span class="chip chip--wait" style="text-align:center"><?php echo $days;?> Day<?php if($days>1){echo "s";}?><br>Pending Action</span>
       <?php } ?> 
</a>
  ...
  <button class="load-more-btn" type="button" data-target="vacation-rows">Load more</button>
...
<?php if (isset($_GET['done'])) { ?> 
    ...
    <div class="hl-modal" id="success-message" role="alertdialog" aria-live="assertive" aria-label="Success">
      <div class="hl-modal__box">
        ...
        <p class="hl-modal__text">Sent successfully</p>
      </div>
    </div>
