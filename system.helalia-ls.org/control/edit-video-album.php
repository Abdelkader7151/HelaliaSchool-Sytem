<?php require_once('includes/access.php'); 
 ...
 if($row_get_login['access14']==1){
    ...
    if(isset($_GET['remove'])){      
          $query_get_gallery = "SELECT * FROM `videos` where `id`='{$_GET['remove']}'  ";
         ...
         $deleteSQL = sprintf("DELETE FROM `videos` WHERE `id`=%s ",
                           GetSQLValueString($database,$_GET['remove'], "int"));
     
          mysqli_query($database,$deleteSQL) or die(mysqli_error($database));
          if($row_get_gallery['video']!=NULL){
              @unlink("../gallery/".$row_get_gallery['video']);
          }
    ...
    if(isset($_GET['del'])){      
        $deleteSQL = sprintf("DELETE FROM `video-albums` WHERE `id`=%s ",
                           GetSQLValueString($database,$_GET['del'], "int"));
    
                mysqli_query($database,$deleteSQL) or die(mysqli_error($database));

          $query_get_gallery = "SELECT * FROM `videos` where `album_id`='{$_GET['del']}'  ";
          $get_gallery  = mysqli_query($database,$query_get_gallery) or die(mysqli_error($database));
          $row_get_gallery  = mysqli_fetch_assoc($get_gallery);
          $totalRows_get_gallery = mysqli_num_rows($get_gallery);
          if($totalRows_get_gallery>0){
              do{
                  if($row_get_gallery['video']!=NULL){
                      @unlink("../gallery/".$row_get_gallery['video']);
                  }
              }while($row_get_gallery  = mysqli_fetch_assoc($get_gallery));
          }

         $deleteSQL2 = sprintf("DELETE FROM `videos` WHERE `album_id`=%s ",
                           GetSQLValueString($database,$_GET['del'], "int"));
     
          mysqli_query($database,$deleteSQL2) or die(mysqli_error($database));
       
                header("location: all-video-albums.php"); 
                exit(); 
        } 
...
if(isset($_POST['submit'])){    
        ...
        $insertSQL = sprintf("UPDATE `video-albums` SET `year`=%s, `title`=%s WHERE `id` = %s", 
       ...
       mysqli_query($database,$insertSQL) or die(mysqli_error($database)); 
             ...
             $deleteSQL = sprintf("DELETE FROM `video-albums-classs` WHERE `album_id`=%s ",
                           GetSQLValueString($database,$_POST['id'], "int"));
...
$query_get_gallery_albums = "SELECT * FROM `video-albums` where `id`='{$_GET['id']}'  ";
$get_gallery_albums = mysqli_query($database,$query_get_gallery_albums) or die(mysqli_error($database));
$row_get_gallery_albums = mysqli_fetch_assoc($get_gallery_albums);
$totalRows_get_gallery_albums= mysqli_num_rows($get_gallery_albums);

if($totalRows_get_gallery_albums<1){header("location: all-video-albums.php"); exit();}



mysqli_select_db($database, $database_database); 
$query_get_gallery = "SELECT * FROM `videos` where `album_id`='{$_GET['id']}'  ";
$get_gallery  = mysqli_query($database,$query_get_gallery) or die(mysqli_error($database));
$row_get_gallery  = mysqli_fetch_assoc($get_gallery);
$totalRows_get_gallery = mysqli_num_rows($get_gallery);
  ...
  <body class="layout layout-header-fixed"> 
        ...
        <div class="layout-content-body"> 
			...
			<div class="row">
				<div class="col-md-8">
				  <div class="demo-form-wrapper">
                    <form action="edit-video-album.php?id=<?php echo $_GET['id'];?>" method="post" name="form1" id="demo-inputmask"  enctype="multipart/form-data" class="form form-horizontal">
           ...
           <div class="form-group">
                ...
                <select class="form-control" required name="year" id="year" >
                      ...
                      <option value="8" <?php if($row_get_gallery_albums['year']==8){echo " selected ";}?> >  الصف السادس الابتدائى</option> 
    ...
    $totalRows_get_data = mysqli_num_rows($get_data); ?>
           ...
           <div class="form-group">
              ...
              <div class="col-sm-6"  id="study_year_class">
                  <?php if($totalRows_get_data>0){
                        do{
                          $query_get_classs = "SELECT * FROM `video-albums-classs` WHERE `album_id`='{$row_get_gallery_albums['id']}' AND `class_id`='{$row_get_data['id']}' ";
           ...
           <div class="form-group">
                ...
                <div class="col-sm-2"> <a href="add-video.php?id=<?php echo $row_get_gallery_albums['id'];?>" class="btn btn-danger btn-block"  ><i class="fa fa-plus" aria-hidden="true"></i> اضافة   </a></div> 

              <div class="col-sm-3"> </div> 
                <div class="col-sm-2"> 
                <?php if(check_video_gallery($row_get_gallery_albums['id'])==0){?>  
                <a href="edit-video-album.php?del=<?php echo $row_get_gallery_albums['id'];?>" class="btn btn-danger btn-block" onclick="return confirm('تاكيد الحذف؟');" ><i class="fa fa-trash-o" aria-hidden="true"></i> حذف   </a>
                  <?php }?>   
                </div>
            </div> 
             ...
             <div class="form-group">       
             <label class="col-sm-3 control-label" for="picture"> </label>    
             <div class="col-sm-9">
                      
 
                        <?php  do{?>
<div class="col-sm-3" style="height:250px; padding: 10px; margin-bottom: 30px; text-align: center;">
<?php  if($row_get_gallery['video']==NULL){echo $row_get_gallery['link'];} if($row_get_gallery['video']!=NULL){?>
   <video style=" max-height: 200px; max-width: 100%;"   controls>
    <source src="../gallery/<?php echo $row_get_gallery['video'];?>" type="video/mp4"> 
  </video>

 
  <?php }?>
    <a href="edit-video-album.php?remove=<?php echo $row_get_gallery['id'];?>&id=<?php echo $_GET['id'];?>" style="width: 100%;"  class="btn btn-danger btn-block" onclick="return confirm('تاكيد الحذف؟');" ><i class="fa fa-trash-o" aria-hidden="true"></i> حذف   </a>
</div>     
