...
if(isset($_POST['submit'])){   
  ...
  $Result2 = mysqli_query($database,$insertSQL2) or die(mysqli_error($database)); 
        ...
        if($_POST['target']==300){ 
          ...
          $where = " AND `teachers`.study_year = '{$_POST['target']}' ";
        }

        if($_POST['class']<1){ 
          $class  = ""; 
        }else{
          $class = " AND `teachers`.class = '{$_POST['class']}' ";
        }
        $query_get_target = "SELECT `teachers`.emp_id AS `emp_id`, `teachers`.class AS `class`, `teachers`.study_year AS `study_year`,  `app_login`.phone_id  AS `phone_id`, `app_login`.id  AS `app_login_id` FROM `teachers` RIGHT JOIN `app_login` ON `teachers`.emp_id = `app_login`.emp_id WHERE `teachers`.id > 0  $where $class   ";
            ...
            do{ 
                $insertSQL1 = sprintf("INSERT INTO `notifications` (`not_id`, `image`, `title`, `user_id`, `text`, `date`, `type`) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                        GetSQLValueString($database,$row_get_push_msg['id'], "int"),
                                GetSQLValueString($database,$image_name, "text"),
                                GetSQLValueString($database,$_POST['title'], "text"),
                                GetSQLValueString($database,$row_get_target['emp_id'], "int"),
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
			...
			<div class="row"> 
				<div class="col-md-6">
				  <div class="demo-form-wrapper"> 
                    <form action="new_push_emp.php" method="post"   name="form1" id="demo-inputmask2"  enctype="multipart/form-data" class="form form-horizontal">
            ...
            <div class="form-group">
						<label class="col-sm-3 control-label" for="study_year"> المستهدف  </label>
						<div class="col-sm-4">
