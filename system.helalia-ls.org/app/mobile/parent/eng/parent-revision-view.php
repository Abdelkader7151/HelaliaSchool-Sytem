...
<div class="app">
  <header class="hero hero--tall">
    <div class="hero__row">
      <a class="back" href="parent-revision.php?id=<?php echo $kid_id;?>" aria-label="Back">
  ...
  <main class="page">
    ...
    <div class="rows">
      <?php
        $query_get_data_today = "SELECT * FROM `revision` WHERE `study_year` = '{$row_get_kid_data['study_year']}' AND (`class` = '{$row_get_kid_data['class']}' ||  `class` = 0 ) AND `confirm` = 1 AND `subject` = '{$id}' AND `date` ='{$today}' order BY `id` desc  ";
        ...
        if($totalRows_get_data_today>0){
          do{ ?> 
          <a class="row t-gold" href="parent-revision-item.php?id=<?php echo $row_get_data_today['id'];?>&kid=<?php echo $row_get_kid_data['id'];?>">
            <span class="row__ico">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
      ...
      <?php }while($row_get_data_today = mysqli_fetch_assoc($get_data_today));
        ...
        <a class="row " href="#">No revision found</a>    
        <?php } ?>     
  </div>
 ...
 <div class="rows">
      <?php
        $query_get_data_old = "SELECT * FROM `revision` WHERE `study_year` = '{$row_get_kid_data['study_year']}' AND (`class` = '{$row_get_kid_data['class']}' ||  `class` = 0 ) AND `confirm` = 1 AND `subject` = '{$id}' AND `date` ='{$today}' order BY `id` desc  ";
      ...
      <?php
          do{ ?>
            <a class="row t-green hw--old" href="parent-revision-item.php?id=<?php echo $row_get_data_old['id'];?>&kid=<?php echo $row_get_kid_data['id'];?>">
