...
mysqli_query($database,"set names 'utf8'"); 
 ...
 function app_id($id){   
    $query_get_data = " SELECT * FROM `app_login` where `emp_id`= '{$id}' "; 
...
function add_notifications($user_id,$emp_id,$text,$type){  
    $insertSQL = sprintf("INSERT INTO `notifications` ( `user_id`,  `text`, `type`, `date`) VALUES (   %s, %s, %s, %s)", 
...
$query_get_users_info = " SELECT * FROM `emps` "; 
   ...
   if($totalRows_get_users_info>0){ 
      do{ 
           if(birthday_check($row_get_users_info['birthday'])==1){  
                  ...
                  add_notifications(app_id($row_get_users_info['id']),$row_get_users_info['id'],"Happy Birthday ".$row_get_users_info['name'],2); 
           }
