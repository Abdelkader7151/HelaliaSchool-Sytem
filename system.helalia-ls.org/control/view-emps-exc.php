<?php require_once('includes/access.php'); 
 ...
 if($row_get_login['access6sub6']==1){
  ...
  if(isset($_GET['del'])){     
    $deleteSQL = sprintf("DELETE FROM `emps_excuse` WHERE `id`=%s ",
...
if(isset($_POST['submit'])){  
	$insertSQL = sprintf("UPDATE `emps_excuse` SET `status`=%s WHERE `id`=%s ", 
       ...
       $Result1 = mysqli_query($database,$insertSQL) or die(mysqli_error($database));
        ...
        header("location: view-emps-exc.php?id=".$_POST['id']."&done"); 
        exit(); 
	} 
 
  mysqli_select_db($database, $database_database); 
  $query_get_emps_vacations = "SELECT * FROM `emps_excuse`  where `id`='{$_GET['id']}'";
			...
			<div class="row">
				<div class="col-md-8">
				  <div class="demo-form-wrapper">
                    <form action="view-emps-exc.php?id=<?php echo $_GET['id'];?>" method="post" name="form1" id="demo-inputmask"  enctype="multipart/form-data" class="form form-horizontal">
                    ...
                    <div class="form-group">
                        <label class="col-sm-3 control-label" for="job">  اليوم  </label>
                        <div class="col-sm-4">
                            <input id="job" class="form-control" readonly type="text" name="date" style=" background-color:white" value="<?php echo date("d/m/Y",$row_get_emps_vacations['date']);?>">
                        </div>
                    </div> 
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2">
                            <input id="start" class="form-control" readonly type="text" name="start" style=" background-color:white" value="<?php echo date("h:i a",$row_get_emps_vacations['start']);?>">
                        </div>
              
                        <label class="col-sm-1 control-label" for="job">  الي  </label>
                        <div class="col-sm-2">
                            <input id="end" class="form-control" readonly type="text" name="end" style=" background-color:white" value="<?php echo date("h:i a",$row_get_emps_vacations['end']);?>">
                        </div>

                        <label class="col-sm-1 control-label" for="job">  المدة  </label>

                        <div class="col-sm-2">
                            <input id="end" class="form-control" readonly type="text" name="end" style=" background-color:white; text-align:center" value="<?php echo $row_get_emps_vacations['hours'];?>">
                        </div>
                        <label class="col-sm-1 control-label" for="job">  ساعة  </label>
                     </div> 
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2"> 
                    <a href="view-emps-exc.php?del=<?php echo $_GET['id'];?>" class="btn btn-danger btn-block" onclick="return confirm('تاكيد الحذف؟');" ><i class="fa fa-trash-o" aria-hidden="true"></i> حذف   </a>
                    </div>
                </div> 
