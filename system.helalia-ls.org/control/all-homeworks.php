  ...
  if(isset($_GET['del'])){   
    ...
    if($row_get_class_info['banner']!=NULL){ unlink('../homework/'.$row_get_class_info['banner']); }
       ...
       $deleteSQL = sprintf("DELETE FROM `homework` WHERE `id`=%s ",
        ...
        header("location: all-homeworks.php"); 
        exit(); 
} 
...
mysqli_select_db($database, $database_database);  
if(isset($_GET['all'])){
  $query_get_class_info = "SELECT * FROM `homework` ORDER BY `id` DESC "; 
 }else{ 
   $query_get_class_info = "SELECT * FROM `homework` WHERE `start` = '{$start}' ";
} 
$get_class_info = mysqli_query($database,$query_get_class_info) or die(mysqli_error($database));
$row_get_class_info = mysqli_fetch_assoc($get_class_info);
$totalRows_get_class_info = mysqli_num_rows($get_class_info);
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
            ...
            <div class="col-md-12">
                <h4 style="padding-right: 20px;">عرض جميع الواجبات المدرسية لليوم   
                  <br>
                          ...
                          <div class="dropdown">
                            <button class="btn btn-primary btn-xs dropdown-toggle" data-toggle="dropdown" type="button" aria-expanded="false">
                              ...
                              <ul class="dropdown-menu dropdown-menu-right"> 
                                 <li><a href="all-homeworks.php?del=<?php echo $row_get_class_info['id'];?>" onClick="return confirm('تاكيد؟');">  <i class="fa fa-trash-o" aria-hidden="true" style="color: red"></i>  حذف </a></li>
                             </ul>
                          </div>
                        </td>
                      </tr>
