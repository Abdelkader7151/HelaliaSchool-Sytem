...
$msg ='';
    ...
    $query_get_class_info = "SELECT * FROM `class` where `id`='{$_GET['id']}'  ";
  ...
  if($totalRows_get_class_info<1){header("location: all-classs.php"); exit();}

  
if(isset($_POST['submit'])){  
    
    mysqli_select_db($database, $database_database); 
    $query_get_kids_list = "SELECT `id`, `name` FROM `kids` where `class`='{$_GET['id']}' AND `linked` = 1  ";
    ...
    $totalRows_get_kids_list = mysqli_num_rows($get_kids_list);
      if($totalRows_get_kids_list>0){
          do{ 
            mysqli_select_db($database, $database_database); 
            $query_get_parent_id = "SELECT `parent_id` FROM `kids_list` where `kid_id`='{$row_get_kids_list['id']}'";
        ...
        $message = $row_get_kids_list['name'];
        $message .=' '.$_POST['msg'];

            $insertSQL1 = sprintf("INSERT INTO `notifications` (`user_id`, `kid_id`, `text`, `date`, `type`) VALUES (%s, %s, %s, %s, %s)",
                                    GetSQLValueString($database,$row_get_parent_id['parent_id'], "int"),
                                    GetSQLValueString($database,$row_get_kids_list['id'], "int"),
         ...
         if(app_msg_id($row_get_kids_list['id'])!=null){sendMessage(app_msg_id($row_get_kids_list['id']),'HLS',$message);}

          }while($row_get_kids_list = mysqli_fetch_assoc($get_kids_list));

          $insertSQL2 = sprintf("INSERT INTO `class_announce` ( `class_id`, `msg`, `date`, `admin_id`) VALUES (%s, %s, %s, %s)", 
                    GetSQLValueString($database,$_GET['id'], "int"),
                    GetSQLValueString($database,$_POST['msg'], "text"),
                    GetSQLValueString($database,time(), "int"),
			...
			header("location: class-announce.php?id=".$_GET['id']."&updated"); 
			exit(); 
	} 
  ...
  mysqli_select_db($database, $database_database); 
  $query_get_class = "SELECT * FROM `class`  where `study_year`='{$row_get_class_info['study_year']}'";
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
			...
			<div class="row"> 
				<div class="col-md-6">
				  <div class="demo-form-wrapper"> 
                    <form action="class-announce.php?id=<?php echo $_GET['id'];?>" method="post" name="form1" id="demo-inputmask"  enctype="multipart/form-data" class="form form-horizontal">
		    ...
		    <div class="form-group">
              ...
              <div class="col-sm-6">
                <input id="name" class="form-control"  disabled   type="text" name="name" value="<?php echo $row_get_class_info['name'];?>">
              </div>
            </div>  
            ...
            <div class="form-group">
                ...
                <select class="form-control" disabled name="study_year" id="study_year" >
                      ...
                      <option value="8" <?php if($row_get_class_info['study_year']==8){echo " selected ";}?> >  الصف الخامس الابتدائى</option> 
           ...
           <div class="col-xs-12">
            ...
            <table id="court-datatables" class="table table-striped   dataTable" width="100%" style="font-size: 16px; ">
                   ...
                   <?php
                     mysqli_select_db($database, $database_database); 
                     $query_get_requests_attend = "SELECT * FROM `class_announce` WHERE `class_id`='{$_GET['id']}'";
                     ...
                     if($totalRows_get_requests_attend>0){
                      ...
                      <tr>
                        ...
                        <td><?php if($row_get_requests_attend['admin_id']!=null){echo users_name($row_get_requests_attend['admin_id']);}else{echo emp_name($row_get_requests_attend['emp_id']);}?></td>
                      </tr>
