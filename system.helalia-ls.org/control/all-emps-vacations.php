...
if(isset($_GET['id'])){
  $query_get_kids_vacations = "SELECT * FROM `emps_vacations` where `status` = '{$_GET['id']}' order by `date` asc  "; 
}else{
  $query_get_kids_vacations = "SELECT * FROM `emps_vacations` order by `date` asc  "; 
} 
    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
        <div class="layout-content-body"> 
			<div class="row">
            <div class="col-md-12">
                <h4>عرض الاجازات <?php if(isset($_GET['id'])){echo "الجديدة";}?> للموظفين   </h4>  
            </div>
          </div>
                ...
                <table id="court-datatables" class="table table-striped   dataTable" width="100%" style="font-size: 16px;  ">
                    ...
                    <tbody>
						...
						<?php if($totalRows_get_kids_vacations>0){
	                           do{  ?>
                      <tr class="count"> 
                        <td class="text-left"><?php echo emp_name($row_get_kids_vacations['emp_id']);?></td>  
                        <td class="text-center"><?php echo job_name(emp_job($row_get_kids_vacations['emp_id']));?></td>  
                        <td class="text-center"><?php echo date("m/d/Y",$row_get_kids_vacations['vacation_start']);?></td>  
                        <td class="text-center"><?php echo date("m/d/Y",$row_get_kids_vacations['vacation_end']);?></td>  
                        <td class="text-center"> <?php echo vac_days($row_get_kids_vacations['vacation_start'],$row_get_kids_vacations['vacation_end']);?> </td> 
                        <td class="text-center"> <?php echo vac_type($row_get_kids_vacations['type']);?> </td>  
                        ...
                        <a href="view-emps-vacation.php?id=<?php echo $row_get_kids_vacations['id'];?>" class="btn btn-info" >    عرض </a>
                        </td>
                      </tr>
 ...
 <script>
	  $(document).ready(function(){ 
		...
		$("#court-datatables").DataTable({ 
            ...
            "aoColumnDefs": [
               { "bSortable": false,
			     "aTargets": [ <?php if(!isset($_GET['id'])){echo 8;}else{echo 7;}?> ]  
			   }
			 ]
        }); 
