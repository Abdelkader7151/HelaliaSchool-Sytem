    ...
    <div class="layout-main">
      ...
      <div class="layout-content">
       ...
       <?php }else{  
            ...
            $query_get_data = "SELECT * FROM `kids` WHERE `study_year` = '{$_GET['year']}' $gender $class  ORDER BY  `gender` DESC, `name` ASC ";    
        ...
        ?>
      	<div class="row">
				<div class="col-md-12">     
        <h3 style="padding-right: 30px; font-size:18px; margin-top:15px"><span  style="color:brown">السنة الدراسية:</span>   <?php echo year_of_study($_GET['year']);?></h3>
        <h3 style="padding-right: 30px; font-size:18px; margin-top:15px"><span  style="color:brown">  الفصل:</span>           <?php echo class_name($_GET['class']);?></h3> 
        <h3 style="padding-right: 30px; font-size:18px; margin-top:15px"><span  style="color:brown">  شهر:</span>             <?php echo $_GET['month'];?></h3> 
        <h3 style="padding-right: 30px; font-size:18px; margin-top:15px"><span  style="color:brown">  النوع:</span>           <?php  if($_GET['gender']!='الاثنين'){echo $_GET['gender'];}else{echo " ولاد / بنات";}?></h3> 
                ...
                <table id="court-datatables" class="table table-striped table-nowrap dataTable" cellspacing="0" width="100%" style="font-size: 16px">
                    <thead>
                      <tr>  
                       ...
                       <th class="text-center">   <?php echo $row_get_subject_header['name']."(".$row_get_subject_header['id'].")<br>".$row_get_subject_header['score'];?></th>  
                    ...
                    <tbody>
                      ...
                      <td class="text-left" style=" border:solid 1px black"><?php echo $row_get_data['name'];?></td> 
                      <td class="text-left" style=" border:solid 1px black"><?php echo $row_get_data['gender'];?></td> 
                      ...
                      <?php 
                        ...
                        if($totalRows_get_subject_result>0){
                          ...
                          do{
                            ...
                            mysqli_select_db($database, $database_database);  
                            $query_get_users_info = "SELECT * FROM `control` WHERE `kid_id` ='{$row_get_data['id']}' AND  `study_year`= '{$_GET['year']}' AND  `subject_id`='{$row_get_subject_result['id']}' AND `month`='{$_GET['month']}'    ";    
                          ...
                          if($row_get_users_info['confirm_by']<1){ $stop++;  }
                        
                        }while($row_get_subject_result = mysqli_fetch_assoc($get_subject_result)); 
                         }?> 
                      ...
                      <?php  if($row_get_login['access23sub3']==1){ ?>
                         <td class="text-center">  
                          <input type="checkbox" style="text-align: center;" id="confirm_<?php echo $row_get_data['id'];?>" class="confirm"  value="1" <?php if($stop==0){echo " checked ";} ?>    >
