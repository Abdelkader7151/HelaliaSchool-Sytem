<?php require_once('../../Connections/database.php');
      ...
      if($totalRows_get_kid_data==0){
        ...
        $query_get_data = "SELECT * FROM `revision` WHERE `confirm` = 1 AND `id` = '{$id}' ";
...
<div class="app">
  <header class="hero hero--tall">
    ...
    <div class="hero__row">
      <a class="back" href="parent-revision-view.php?id=<?php echo $row_get_data['subject'];?>&kid=<?php echo $kid_id; ?>" aria-label="Back">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          <path d="M15 19 8 12l7-7"/>
        </svg>
        </a>
          <div class="grow">
            <p class="hero__eyebrow">Revision</p>
...
<div class="card stack">
      ...
      <div class="folds"> 
       ...
       <?php
          $query_get_revision_videos = "SELECT * FROM `revision_attachments` WHERE `revision_id` = '{$id}' AND `type` = 1 ";
          $get_revision_videos = mysqli_query($database ,$query_get_revision_videos) or die(mysqli_error($database));
          $row_get_revision_videos = mysqli_fetch_assoc($get_revision_videos);
          $totalRows_get_revision_videos = mysqli_num_rows($get_revision_videos);
         
          if($totalRows_get_revision_videos > 0){ ?>
                      ...
                      <div class="fold t-coral">     
                        <button class="fold__head" type="button">
                          ...
                          <span class="fold__body">
                            ...
                            <span class="row__meta"><?php echo $totalRows_get_revision_videos;?> clips</span>
                          </span>
                        ...
                        <div class="fold__panel">
                          <div class="shots shots--2">
                           <?php $vid_n = 0; do{ $vid_n++;
                             $vid_path = '../../../../homework/' . $row_get_revision_videos['video'];
                           ...
                           <?php  }while($row_get_revision_videos = mysqli_fetch_assoc($get_revision_videos)); ?> 
         ...
         <?php } 
          ...
          $query_get_revision_picture = "SELECT `picture` FROM `revision_attachments` WHERE `revision_id` = '{$id}' AND `type` = 2 ";
          $get_revision_picture = mysqli_query($database ,$query_get_revision_picture) or die(mysqli_error($database));
          $row_get_revision_picture = mysqli_fetch_assoc($get_revision_picture);
          $totalRows_get_revision_picture = mysqli_num_rows($get_revision_picture);
         
          if($totalRows_get_revision_picture > 0){ ?>
            ...
            <div class="fold t-navy">
              <button class="fold__head" type="button">
                ...
                <span class="fold__body">
                  ...
                  <span class="row__meta"><?php echo $totalRows_get_revision_picture;?> images</span>
                </span>
                <span class="fold__chev">›</span>
              </button>
              <div class="fold__panel">
                <div class="shots">
                  <?php $pic_n = 0; do{ $pic_n++;
                    $pic_path = '';
                    if ($row_get_revision_picture['picture'] != NULL && file_exists('../../../../homework/' . $row_get_revision_picture['picture'])) {
                      $pic_path = '../../../../homework/' . $row_get_revision_picture['picture'];
                    }
                  ...
                  <?php  } while($row_get_revision_picture = mysqli_fetch_assoc($get_revision_picture)); ?> 
                </div>
              </div>
            </div>
 ...
 <?php } 
          ...
          $query_get_revision_links = "SELECT `url` FROM `revision_attachments` WHERE `revision_id` = '{$id}' AND `type` = 3 ";
          $get_revision_links = mysqli_query($database ,$query_get_revision_links) or die(mysqli_error($database));
          $row_get_revision_links = mysqli_fetch_assoc($get_revision_links);
          $totalRows_get_revision_links = mysqli_num_rows($get_revision_links);
         
          if($totalRows_get_revision_links > 0){ ?>
    ...
    <div class="fold t-gold">
      <button class="fold__head" type="button">
        ...
        <span class="fold__body">
          ...
          <span class="row__meta"><?php echo $totalRows_get_revision_links;?> link</span>
        </span>
        <span class="fold__chev">›</span>
      </button>
      <div class="fold__panel">
         <?php do{ ?>
        <a class="fold__item" href="<?php echo $row_get_revision_links['url'];?>" >
          <span class="fold__item-ico fold__item-ico--link">›</span>
          <!--span class="fold__item-title">Practice exercises</span>-->
          <span class="fold__item-meta"><?php echo $row_get_revision_links['url'];?></span>
        </a>
          <?php  } while($row_get_revision_links = mysqli_fetch_assoc($get_revision_links)); ?> 
      </div>
    </div>
     <?php }  
 
          $query_get_revision_files = "SELECT `file` FROM `revision_attachments` WHERE `revision_id` = '{$id}' AND `type` = 4 ";
          $get_revision_files = mysqli_query($database ,$query_get_revision_files) or die(mysqli_error($database));
          $row_get_revision_files = mysqli_fetch_assoc($get_revision_files);
          $totalRows_get_revision_files = mysqli_num_rows($get_revision_files);
         
          if($totalRows_get_revision_files > 0){ ?> 
    ...
    <div class="fold t-green">
     ...
     <button class="fold__head" type="button">
      ...
      <div class="fold__panel">
<?php do{ ?>
        <a class="fold__item" href="../../../../homework/<?php echo $row_get_revision_files['file'];?>">  
          <?php if(strtolower(pathinfo(trim($row_get_revision_files['file']), PATHINFO_EXTENSION))=='pdf'){ echo $pdf; } ?>
          <?php if(strtolower(pathinfo(trim($row_get_revision_files['file']), PATHINFO_EXTENSION))=='doc' || strtolower(pathinfo(trim($row_get_revision_files['file']), PATHINFO_EXTENSION))=='docx' ){ echo $doc; } ?> 
          <?php if(strtolower(pathinfo(trim($row_get_revision_files['file']), PATHINFO_EXTENSION))=='xls' || strtolower(pathinfo(trim($row_get_revision_files['file']), PATHINFO_EXTENSION))=='xlsx' ){ echo $doc; } ?> 
       
          <!--<span class="fold__item-title">Unit 4 worksheet.docx</span>-->
          <span class="fold__item-meta">Download</span>
        </a> 
    <?php  } while($row_get_revision_files = mysqli_fetch_assoc($get_revision_files)); ?>     
      </div>
